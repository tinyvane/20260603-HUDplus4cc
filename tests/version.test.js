...
test('getClaudeCodeVersion executes the resolved binary path', async () => {
  ...
  try {
    ...
    // Force a non-Windows platform so this exercises the direct-execution path
    // deterministically regardless of host OS. On Windows a `.cmd` binary is
    // otherwise wrapped via cmd.exe — that behavior is covered by the dedicated
    // "Windows wrapper invocation" test below.
    _setVersionInvocationEnvForTests(() => 'linux', null);
