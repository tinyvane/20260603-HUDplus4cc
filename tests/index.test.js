...
test("index entrypoint runs when executed directly", async () => {
  ...
  try {
    ...
    // Use fileURLToPath so argv[1] is a native path on every platform. On
    // Windows, URL.pathname yields "/C:/..." which realpathSync cannot resolve,
    // so the entrypoint guard would never match the real script path.
