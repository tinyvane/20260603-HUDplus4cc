...
| `elementOrder` | string[] | `["project","addedDirs","context","usage","promptCache","memory","environment","chats","acpec","submodulePush","tools","agents","todos","sessionTime"]` | Expanded-mode element order. Omit entries to hide them in expanded mode. Existing configs keep their explicit order until updated. |
...
| `display.showChats` | boolean | false | Show a chat-sessions line for the current project: `💬 12` session count with a persisted peak high-water mark |
| `display.chatShowPeakAlert` | boolean | true | When the session count drops below the recorded peak, append a `⚠ (peak N)` alert — a sentinel for sync/data loss |
| `display.chatShowSize` | boolean | false | Append the total size of the project's session files, e.g. `· 3.2MB` |
| `display.chatShowLastActive` | boolean | false | Append how long ago the most recent session was active, e.g. `· 5m ago` |
...
| `chatArchive.backupAll` | boolean | false | When true, `/claude-hud:backup-chats` backs up **every** project by default instead of just the current one. Override per-run with `--all` / `--no-all` |
...
| `acpec.protectedBranches` | string[] | `[]` | Branches ACPEC must never commit to, e.g. `["main", "master"]` |
...
Opt-in tools for keeping conversation history and code in sync across machines (for example when `~/.claude/projects` lives in a Dropbox/Syncthing-synced folder). All of them are disabled by default.

### Chats line

Enable `display.showChats` to see how many Claude Code sessions (`.jsonl` transcripts) the current project has:

```
💬 12 ⚠ (peak 14) · 3.2MB · 5m ago
```

- The count is tracked against a persisted **peak** (high-water mark). If the count ever drops below the peak — a typical symptom of a sync conflict or accidental deletion — a `⚠ (peak N)` alert appears (`display.chatShowPeakAlert`).
- `display.chatShowSize` and `display.chatShowLastActive` append the total transcript size and last activity age.
- With `display.chatClickable` (default on), clicking the counter opens the project's sessions folder in terminals that support OSC 8 hyperlinks.
...
`/claude-hud:acpec on|off|status` manages a **SessionEnd hook** that automatically stages tracked changes (`git add -u`), commits, and pushes the current branch when a conversation ends — so your repos are always synced before you switch machines.
...
The HUD shows the current state via `display.showAcpec` (`ACPEC ✓` / `ACPEC ✗`).

### Submodule push reminder

If a repo tracks nested git repos (gitlinks — including bare gitlinks without a `.gitmodules` file), a plain `git push` from the parent silently skips them, so sync tools see nothing to do. Enable `display.showSubmodulePush` to get a reminder:

```
Submodule ✗ run on-demand     ← fix: git config push.recurseSubmodules on-demand
Submodule ✓ on-demand         ← parent pushes recurse into nested repos
```

The line is hidden entirely when the repo has no gitlinks.
