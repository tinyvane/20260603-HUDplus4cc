...
| `elementOrder` | string[] | `["project","addedDirs","context","usage","promptCache","memory","environment","chats","acpec","submodulePush","tools","agents","todos","sessionTime"]` | 展开模式下元素的顺序。省略的条目在展开模式下隐藏。现有配置会保留其显式顺序直到更新 |
...
| `display.showChats` | boolean | false | 显示当前项目的对话会话行：`💬 12` 会话数量，并持久化记录峰值（高水位线） |
| `display.chatShowPeakAlert` | boolean | true | 当会话数量低于记录的峰值时追加 `⚠ (peak N)` 警告——同步丢失的哨兵信号 |
| `display.chatShowSize` | boolean | false | 追加项目会话文件的总大小，如 `· 3.2MB` |
| `display.chatShowLastActive` | boolean | false | 追加最近一次会话的活跃时间，如 `· 5m ago` |
...
| `chatArchive.backupAll` | boolean | false | 为 true 时，`/claude-hud:backup-chats` 默认备份**全部**项目而非仅当前项目。可用 `--all` / `--no-all` 临时覆盖 |
...
| `acpec.protectedBranches` | string[] | `[]` | ACPEC 永不提交的分支，如 `["main", "master"]` |
...
一组可选工具，用于在多台机器之间保持对话历史和代码同步（例如把 `~/.claude/projects` 放在 Dropbox/Syncthing 同步目录中的场景）。全部默认关闭。

### 会话计数行（Chats）

启用 `display.showChats` 可以看到当前项目有多少个 Claude Code 会话（`.jsonl` transcript）：

```
💬 12 ⚠ (peak 14) · 3.2MB · 5m ago
```

- 会话数量与持久化的**峰值**（高水位线）对比。一旦数量低于峰值——这通常是同步冲突或误删的征兆——就会出现 `⚠ (peak N)` 警告（`display.chatShowPeakAlert`）。
- `display.chatShowSize` 和 `display.chatShowLastActive` 可追加 transcript 总大小和最近活跃时间。
- `display.chatClickable`（默认开启）使计数在支持 OSC 8 超链接的终端中可点击，直接打开项目的会话文件夹。

### 备份与恢复会话

- `/claude-hud:backup-chats` —— 将当前项目的会话 transcript 复制到归档目录（`chatArchive.path`），并显示本地与归档并排对比表。仅当本地文件严格追加于归档时才更新；截短或分叉的本地副本会保存在 `.conflicts/` 下。
- `/claude-hud:recover-chats` —— 从归档恢复本地**缺失**的会话，绝不覆盖已有文件。加 `--all` 可恢复所有项目。

### ACPEC —— 每次对话结束自动提交并推送

`/claude-hud:acpec on|off|status` 管理一个 **SessionEnd 钩子**：对话结束时自动暂存已跟踪的更改（`git add -u`）、提交并推送当前分支——切换机器之前仓库始终是同步好的。

安全契约：

- 只提交**已跟踪**的更改——绝不添加新的未跟踪文件。
- 如果仓库已有暂存内容，ACPEC 会停止而不提交，避免吸收用户的暂存区状态。
- 绝不强制推送；只推送当前分支。
- 跳过非 git 目录、detached HEAD、`acpec.protectedBranches` 中列出的分支，以及根目录位于用户主目录的仓库。
- `on` 会先运行 `.gitignore` 安全门：扫描已被跟踪的敏感文件，并确保敏感基线模式（`.env`、`*.key`、`*secret*` 等）已被忽略，然后才启用。

HUD 通过 `display.showAcpec` 显示当前状态（`ACPEC ✓` / `ACPEC ✗`）。

### 子模块推送提醒

如果仓库跟踪了嵌套 git 仓库（gitlink——包括没有 `.gitmodules` 文件的裸 gitlink），在父仓库执行 `git push` 会静默跳过它们，同步工具因此看似工作实则无效。启用 `display.showSubmodulePush` 可获得提醒：

```
Submodule ✗ run on-demand     ← 修复：git config push.recurseSubmodules on-demand
Submodule ✓ on-demand         ← 父仓库推送会递归到嵌套仓库
```

仓库没有 gitlink 时该行完全隐藏。
