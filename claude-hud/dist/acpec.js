...
function isHomeDirectory(gitToplevel) {
    try {
        ...
        if (process.platform === 'win32')
            return top.toLowerCase() === home.toLowerCase();
        return top === home;
    }
    catch {
        return false;
...
export function runAcpec(opts) {
    ...
    if (!cwd)
        return { action: 'skipped', reason: 'no-cwd' };
    ...
    if (files.length === 0)
        return { action: 'skipped', reason: 'no-changes', branch };
    ...
    if (!commit.ok)
        return { action: 'skipped', reason: 'commit-failed', branch };
...
export function formatAcpecResult(result) {
...
async function resolveCwd(fallback) {
...
export async function mainAcpec() {
