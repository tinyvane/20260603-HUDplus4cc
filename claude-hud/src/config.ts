...
export interface HudConfig {
  ...
  display: {
    ...
    showTodos: boolean;
    showChats: boolean;
    chatShowPeakAlert: boolean;
    chatShowSize: boolean;
    chatShowLastActive: boolean;
...
export const DEFAULT_CONFIG: HudConfig = {
  ...
  display: {
    ...
    showTodos: false,
    showChats: false,
    chatShowPeakAlert: true,
    chatShowSize: false,
    chatShowLastActive: false,
    chatClickable: true,
    showAcpec: false,
  ...
  acpec: {
    enabled: false,
    commitPrefix: 'chore(acpec): auto-sync',
    protectedBranches: [],
  },
  colors: {
    context: 'green',
...
export function mergeConfig(userConfig: Partial<HudConfig>): HudConfig {
  ...
  const display = {
    ...
    showChats: typeof migrated.display?.showChats === 'boolean'
      ? migrated.display.showChats
      : DEFAULT_CONFIG.display.showChats,
    chatShowPeakAlert: typeof migrated.display?.chatShowPeakAlert === 'boolean'
      ? migrated.display.chatShowPeakAlert
      : DEFAULT_CONFIG.display.chatShowPeakAlert,
    chatShowSize: typeof migrated.display?.chatShowSize === 'boolean'
      ? migrated.display.chatShowSize
      : DEFAULT_CONFIG.display.chatShowSize,
    chatShowLastActive: typeof migrated.display?.chatShowLastActive === 'boolean'
      ? migrated.display.chatShowLastActive
      : DEFAULT_CONFIG.display.chatShowLastActive,
    chatClickable: typeof migrated.display?.chatClickable === 'boolean'
      ? migrated.display.chatClickable
      : DEFAULT_CONFIG.display.chatClickable,
    showAcpec: typeof migrated.display?.showAcpec === 'boolean'
      ? migrated.display.showAcpec
      : DEFAULT_CONFIG.display.showAcpec,
  ...
  const acpec = {
    enabled: typeof migrated.acpec?.enabled === 'boolean'
      ? migrated.acpec.enabled
      : DEFAULT_CONFIG.acpec.enabled,
    commitPrefix: typeof migrated.acpec?.commitPrefix === 'string' && migrated.acpec.commitPrefix.trim().length > 0
      ? migrated.acpec.commitPrefix.slice(0, 120)
      : DEFAULT_CONFIG.acpec.commitPrefix,
    protectedBranches: Array.isArray(migrated.acpec?.protectedBranches)
      ? migrated.acpec.protectedBranches.filter((b): b is string => typeof b === 'string')
      : [...DEFAULT_CONFIG.acpec.protectedBranches],
  };

  return { language, lineLayout, showSeparators, pathLevels, maxWidth, forceMaxWidth, elementOrder, gitStatus, display, chatArchive, acpec, colors };
}
