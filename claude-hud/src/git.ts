...
export async function getSubmoduleConfig(cwd?: string): Promise<SubmoduleConfig | null> {
  ...
  try {
    ...
    for (const line of stdout.split('\n')) {
      ...
      if (line.startsWith('160000 ')) count += 1;
    }

    if (count === 0) {
      return { count: 0, recurseValue: null, recurseOnDemand: false };
    }

    let recurseValue: string | null = null;
    try {
    ...
    return { count, recurseValue, recurseOnDemand: recurseValue === 'on-demand' };
  } catch {
    return null;
  }
}
