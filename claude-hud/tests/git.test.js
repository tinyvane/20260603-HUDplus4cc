...
test('getGitStatus keeps line diffs for literal filenames containing arrow text', {
  // The literal filename "foo => bar.txt" contains '>', which is a reserved
  // character on Windows (NTFS/Win32 forbid < > : " / \ | ? *), so the file
  // cannot be created there. The arrow-vs-rename parsing this exercises is
  // pure string handling and is covered cross-platform; skip only the
  // filesystem-dependent fixture on Windows.
  skip: process.platform === 'win32' ? 'filename contains > which is illegal on Windows' : false,
}, async () => {
  const dir = await mkdtemp(path.join(tmpdir(), 'claude-hud-git-'));
  try {
    execFileSync('git', ['init'], { cwd: dir, stdio: 'ignore' });
